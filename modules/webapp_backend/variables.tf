#===================================================================================================================
# Data
# ==================================================================================================================
variable "create" {
  type        = bool
  default     = false
  description = "Whether cluster should be created (affects nearly all resources)"
}
variable "create_ecr_repo" {
  type        = bool
  default     = false
  description = "Whether ECR repository and relevant resources should be created"
}

variable "region" {
  type        = string
  description = "Region i-e: us-east-1, ap-south-1, us-west-2"
}

variable "base_domain_name" {
  type        = string
  description = "Base Domain name:"
}

variable "vpc_id" {
  type        = string
  default     = ""
  description = "VPC Id"
}

variable "private_subnet_ids" {
  type        = list(string)
  default     = []
  description = "IDs of the Private subnets where to create the resources "
}

variable "public_subnet_ids" {
  type        = list(string)
  default     = []
  description = "IDs of the Public subnets where to create the resources "
}

variable "cert_arn_alb" {
  type        = string
  default     = ""
  description = "ARN of the ACM Wildcard DNS Cert"
}

variable "hosted_zone_id" {
  type        = string
  default     = ""
  description = "Hosted zone Id"
}

#===================================================================================================================
# ALB
# ==================================================================================================================
variable "alb_name" {
  type        = string
  default     = ""
  description = "Name of the ALB. If not provided, will use system information(component,region etc.) to generate ALB name."
}

variable "enable_alb_deletion_protection" {
  type        = bool
  default     = true
  description = "Indicates if ALB deletion protection should be enabled"
}

#FIXME - temporarily commented to pass pre-commit check. Once lifecycle rules are enabled again, uncomment this.
#variable "enable_alb_logs_s3_bucket_lifecycle_rules" {
#  type        = bool
#  default     = true
#  description = "Whether to enable lifecycle rules for the S3 bucket"
#}

# ALB S3 Logs
variable "alb_access_logs_s3_bucket_name" {
  type        = string
  default     = ""
  description = "Alb access logs s3 bucket name. If not provided, will use system information(component,region etc.) to generate S3 bucket name."
}

variable "enable_alb_access_logs_s3_bucket_versioning" {
  type        = bool
  default     = true
  description = "Enable alb access logs s3 bucket versioning."
}

variable "alb_access_logs_s3_bucket_object_prefix" {
  type        = string
  default     = "alb-access-logs"
  description = "Alb access logs s3 bucket object prefix."
}

# ECS TG
variable "enable_http_to_https_redirection" {
  type        = bool
  default     = true
  description = "Indicates if HTTP to HTTPS redirection is enabled."
}

variable "target_group_port" {
  type        = number
  default     = 80
  description = "Target group port"
}

variable "target_group_protocol" {
  type        = string
  default     = "HTTP"
  description = "Target group protocol"
}

variable "target_type" {
  type        = string
  default     = "ip"
  description = "Target type i-e IP or instance"
}

#===================================================================================================================
# ROUTE53
# ==================================================================================================================
variable "service_aliases" {
  type        = list(string)
  default     = []
  description = "List of aliases e.g. adserver, thoon. This will be concatenated to base_domain_name and final result would be adserver.<base_domain_name>."
}

variable "evaluate_target_health" {
  type        = bool
  default     = false
  description = "Set to true if you want Route 53 to determine whether to respond to DNS queries"
}

#===================================================================================================================
# Task Definition
# ==================================================================================================================
variable "ecs_task_role" {
  type        = string
  default     = ""
  description = "ECS task role."
}
variable "ecs_task_role_iam_policy" {
  type        = string
  default     = <<-ECS_TASK_ROLE_IAM_POLICY
  {
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Action": [
                "ecr:GetAuthorizationToken",
                "ecr:BatchCheckLayerAvailability",
                "ecr:GetDownloadUrlForLayer",
                "ecr:BatchGetImage",
                "logs:CreateLogStream",
                "logs:PutLogEvents"
            ],
            "Resource": "*"
        }
    ]
  }
ECS_TASK_ROLE_IAM_POLICY
  description = "ECS task role IAM policy."
}

variable "ecs_execution_role" {
  type        = string
  default     = ""
  description = "ECS execution role."
}
variable "ecs_execution_role_iam_policy" {
  type        = string
  default     = <<-ECS_EXECUTION_ROLE_IAM_POLICY
  {
    "Version": "2012-10-17",
    "Statement": [
        {
            "Effect": "Allow",
            "Action": [
                "ecr:GetAuthorizationToken",
                "ecr:BatchCheckLayerAvailability",
                "ecr:GetDownloadUrlForLayer",
                "ecr:BatchGetImage",
                "logs:CreateLogStream",
                "logs:PutLogEvents"
            ],
            "Resource": "*"
        }
    ]
  }
ECS_EXECUTION_ROLE_IAM_POLICY
  description = "ECS execution role IAM policy."
}

variable "task_definition_name" {
  type        = string
  default     = ""
  description = "Name of the task definition. If not provided, will use system information(component,region etc.) to generate task definition name."
}

variable "ecs_task_ephemeral_storage" {
  type        = number
  default     = 21
  description = "ECS task ephemeral storage"
  validation {
    condition     = var.ecs_task_ephemeral_storage > 20 || var.ecs_task_ephemeral_storage < 201
    error_message = "Please specify a value between 21 and 200 for ECS task ephemeral storage."
  }
}

variable "ecs_task_memory_size" {
  type        = number
  default     = 2048
  description = "Amount of memory to allocate to the task."
}

variable "ecs_task_cpu_size" {
  type        = number
  default     = 1024
  description = "Amount of cpu to allocate to the task."
}

variable "ecs_task_primary_container_name" {
  type        = string
  default     = ""
  description = "Name of the Container. If not provided, component name will be used."
}

variable "use_custom_container_definitions" {
  type        = bool
  default     = false
  description = "Indicates if default container definitions should be ignored and `ecs_task_container_definitions` should be utilized for container definitions."
}
variable "ecs_task_container_definitions" {
  type        = string
  default     = ""
  description = "container definitions in heredoc syntax"
}

variable "log_retention_days" {
  type        = number
  default     = 14
  description = "Number of days to retain log events."
}

variable "image_uri" {
  type        = string
  description = "ECR image URI"
}

variable "image_container_port" {
  type        = number
  default     = 80
  description = "Container port"
}

#===================================================================================================================
# ECR
# ==================================================================================================================
variable "ecr_repo_name" {
  type        = string
  default     = ""
  description = "ECR repository name. If not provided, will use system information(component,region etc.) to generate ECR repo name."
}

variable "is_image_tag_immutable" {
  type        = bool
  default     = false
  description = "A boolean that indicates whether the tag immutability setting for the repository should be MUTABLE (true) or IMMUTABLE (false)."
}

variable "ecr_repo_policy" {
  type        = string
  default     = <<-ECR_REPO_POLICY
    {
        "Version": "2012-10-17",
        "Statement": [
          {
            "Sid": "ECRImageRetentionPolicy",
            "Effect": "Allow",
            "Principal": {
              "AWS": "arn:aws:iam::444455556666:root"
            },
            "Action": [
              "ecr:GetDownloadUrlForLayer",
              "ecr:BatchGetImage",
              "ecr:BatchCheckLayerAvailability",
              "ecr:PutImage",
              "ecr:InitiateLayerUpload",
              "ecr:UploadLayerPart",
              "ecr:CompleteLayerUpload",
              "ecr:DescribeRepositories",
              "ecr:GetRepositoryPolicy",
              "ecr:ListImages",
              "ecr:DeleteRepository",
              "ecr:BatchDeleteImage",
              "ecr:SetRepositoryPolicy",
              "ecr:DeleteRepositoryPolicy"
            ]
          }
        ]
      }
ECR_REPO_POLICY
  description = "ECR repository policy."
}
variable "ecr_lifecycle_policy" {
  type        = string
  default     = <<-ECR_LIFECYCLE_POLICY
    {
        "rules": [
            {
                "rulePriority": 1,
                "description": "Keep last (n) images",
                "selection": {
                    "tagStatus": "any",
                    "countType": "imageCountMoreThan",
                    "countNumber": 30
                },
                "action": {
                    "type": "expire"
                }
            }
        ]
    }
ECR_LIFECYCLE_POLICY
  description = "ECR lifecycle policy."
}


variable "ecr_repo_encryption_type" {
  type        = string
  default     = "AES256"
  description = "The encryption type to use for the repository. Valid values are `AES256` or `KMS`"
}

# KMS key
variable "kms_key" {
  type        = string
  default     = ""
  description = "The Amazon Resource Name (ARN) of the KMS Key for encryption."
}

# ECS Target Group
variable "deregistration_delay" {
  type        = number
  default     = 5
  description = "ALB de-registration delay."
}

variable "alb_health_check_interval" {
  type        = number
  default     = 30
  description = "Health check interval"
}

variable "alb_health_check_path" {
  type        = string
  default     = "/"
  description = "Health check path"
}

variable "alb_health_check_port" {
  type        = number
  default     = 80
  description = "Health check port"
}

variable "alb_health_check_protocol" {
  type        = string
  default     = "HTTP"
  description = "Health check protocol"
}

variable "alb_health_check_timeout" {
  type        = number
  default     = 3
  description = "Health check timeout"
}

variable "alb_health_check_unhealthy_threshold" {
  type        = number
  default     = 3
  description = "Health check unhealthy threshold"
}

# ECS Service
variable "ecs_cluster_name" {
  type        = string
  default     = ""
  description = "To override the Default value, please enter a name of the Cluster i-e: test-cluster"
}

variable "enable_container_insights" {
  type        = bool
  default     = true
  description = "Default container insights are enable for ECS cluster"
}

variable "ecs_service_name" {
  type        = string
  default     = ""
  description = "Name of the ECS service."
}
#
#variable "ecs_launch_type" {
#  type        = string
#  default     = "FARGATE"
#  description = "ECS service type"
#}

variable "ecs_deployment_minimum_healthy_percent" {
  type        = number
  default     = 100
  description = "Lower limit (as a percentage of the service's desiredCount) of the number of running tasks that must remain running and healthy in a service during a deployment"
}

variable "ecs_deployment_maximum_percent" {
  type        = number
  default     = 200
  description = "Upper limit (as a percentage of the service's desiredCount) of the number of running tasks that can be running in a service during a deployment. Not valid when using the DAEMON scheduling strategy"
}

variable "task_definition_arn" {
  type = string
  default = ""
  description = "Please provide the task definition arn if you want to create tasks with specific definition revision"
}

variable "enable_ecs_service_wait_for_steady_state" {
  type        = bool
  default     = true
  description = "If true, Terraform will wait for the service to reach a steady state (like aws ecs wait services-stable) before continuing. Default false"
}

# Rollback
variable "enable_ecs_service_deployment_circuit_breaker" {
  type        = bool
  default     = false
  description = "Whether to enable the deployment circuit breaker logic for the service"
}

variable "enable_ecs_service_deployment_rollback" {
  type        = bool
  default     = false
  description = "Whether to enable Amazon ECS to roll back the service if a service deployment fails. If rollback is enabled, when a service deployment fails, the service is rolled back to the last deployment that completed successfully"
}

#capacity provider
variable "enable_fargate_capacity_provider" {
  description = "Whether to enable Fargate capacity provider"
  default     = true
  type        = bool
}
variable "enable_fargate_spot_capacity_provider" {
  description = "Whether to enable Fargate Spot capacity provider"
  default     = false
  type        = bool
}

variable "fargate_capacity_provider_base" {
  type        = number
  default     = 1
  description = "capacity provider base"
}

variable "fargate_capacity_provider_weight" {
  type        = number
  default     = 5
  description = "capacity provider weight"
}

variable "fargate_spot_capacity_provider_base" {
  type        = number
  default     = 0
  description = "capacity provider base"
}

variable "fargate_spot_capacity_provider_weight" {
  type        = number
  default     = 20
  description = "capacity provider weight"
}

#ECS SG
variable "ecs_service_sg_from_port" {
  description = "The starting port for the range of ports to be opened (this is for ECS's service sg)"
  type        = number
  default     = 80
}

variable "ecs_service_sg_to_port" {
  description = "The ending port for the range of ports to be opened (this is for ECS's service sg)"
  type        = number
  default     = 80
}

# Auto Scaling

variable "enable_ecs_service_auto_scaling" {
  type        = bool
  default     = false
  description = "Indicates if ECS service auto-scaling is enabled."
}

#variable "use_cpu_for_scaling" {
#  description = "If true, use CPU for auto-scaling, otherwise use Memory"
#  type        = bool
#  default     = true
#}

variable "desired_count" {
  type        = number
  default     = 1
  description = "Number of instances of the task definition"
}

variable "ecs_service_autoscaling_min_capacity" {
  type        = number
  default     = 1
  description = "The min capacity of the scalable target"
}

variable "ecs_service_autoscaling_max_capacity" {
  type        = number
  default     = 5
  description = "The max capacity of the scalable target"
}

# Scale Up Policy
# CPU
variable "ecs_service_autoscaling_threshold_percentage" {
  type        = number
  default     = 90
  description = "The target value for the metric, in this case cpu/memory i-e: 90"
}

# Cool down Policy
variable "ecs_service_autoscaling_scale_in_cool_down" {
  type        = number
  default     = 300
  description = "The amount of time, in seconds, after a scale in activity completes before another scale in activity can start."
}

variable "ecs_service_autoscaling_scale_out_cool_down" {
  type        = number
  default     = 300
  description = "The amount of time, in seconds, after a scale out activity completes before another scale out activity can start."
}

# Application Health Check
variable "ecs_service_health_check_grace_period_seconds" {
  type        = number
  default     = 180
  description = "Seconds to ignore failing load balancer health checks on newly instantiated tasks to prevent premature shutdown, up to 2147483647. Only valid for services configured to use load balancers."
}

variable "ecs_service_alb_container_port" {
  type        = number
  default     = 80
  description = "Port on the container to associate with the load balancer"
}

#===================================================================================================================
# SSM Parameter Store
# ==================================================================================================================
variable "create_string_parameters" {
  type        = bool
  default     = false
  description = "whether to enable string parameters support or not"
}

variable "create_string_list_parameters" {
  type        = bool
  default     = false
  description = "whether to enable stringlist parameters support or not"
}

variable "create_secure_string_parameters" {
  type        = bool
  default     = false
  description = "whether to enable  secure string parameters support or not"
}

variable "string_parameters" {
  type = list(object({
    key         = string
    value       = string
    description = string
  }))
  default     = []
  description = "List of parameters to be added as String."
}

variable "string_list_parameters" {
  type = list(object({
    key         = string
    value       = string
    description = string
  }))
  default     = []
  description = "List of parameters to be added as StringList."
}

variable "secure_string_parameters" {
  type = list(object({
    key         = string
    value       = string
    description = string
  }))
  default     = []
  description = "List of parameters to be added as SecureString."
}
variable "kms_key_id_for_secure_string" {
  type        = string
  default     = ""
  description = "KMS key ID for secure string i.e alias/aws/ssm.."
}

variable "enable_kms_encryption_for_aws_cloudwatch_log_group" {
  type        = bool
  default     = false
  description = "whether to enable kms encryption for cloudwatch log group or not"
}

variable "kms_key_arn_for_cloudwatch" {
  type        = string
  default     = ""
  description = "arn for cloudwatch kms key"
}

#===================================================================================================================
# Secrets Manager
# ==================================================================================================================
#variable "create_secret_manager" {
#  type        = bool
#  default     = false
#  description = "whether to create secret manager service or not"
#}
#
#variable "secret_manager_name" {
#  type        = string
#  default     = ""
#  description = "secret manager name"
#}
#
#variable "secrets_manager_key_value_pairs" {
#  type        = map(any)
#  default     = {}
#  description = "pair of key and value"
#}
#
#variable "secret_manager_kms_key_id" {
#  type        = string
#  default     = ""
#  description = "kms key id for secret manager"
#}
variable "create_secrets" {
  type        = bool
  default     = false
  description = "Boolean flag to indicate if secrets in secrets manager should be created"
}
variable "secrets" {
  type = list(object({
    description             = string
    kms_key_id              = string
    name                    = string
    recovery_window_in_days = number
    secret_string           = string
  }))
  default     = []
  description = "List of objects where each object contains complete information about a secret"
}
variable "propagate_ecs_service_tags" {
  type        = string
  default     = "SERVICE"
  description = "Specifies whether to propagate the tags from the task definition or the service to the tasks. The valid values are SERVICE and TASK_DEFINITION."
}
variable "enable_execute_command" {
  type        = bool
  default     = false
  description = "Specifies whether to enable Amazon ECS Exec for the tasks within the service."
}

variable "ecs_service_autoscaling_dimension" {
  type        = string
  default     = "cpu"
  description = "Dimension for service auto-scaling"
  validation {
    condition     = var.ecs_service_autoscaling_dimension == "cpu" || var.ecs_service_autoscaling_dimension == "memory" || var.ecs_service_autoscaling_dimension == "alb"
    error_message = "Please specify a valid value i.e. cpu, memory or alb."
  }
}

variable "ecs_service_autoscaling_dimensions" {
  type = map(string)
  default = {
    cpu    = "ECSServiceAverageCPUUtilization"
    memory = "ECSServiceAverageMemoryUtilization"
    alb    = "ALBRequestCountPerTarget"
  }
  description = "mapping for predefined metrics for service auto-scaling"
}
variable "project_name" {
  type        = string
  default     = ""
}

variable "account_id" {
  type = string
}
variable "vpc_cidr" {
  type = string
}